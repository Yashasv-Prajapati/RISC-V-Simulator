...
def fetch(pipe1, out1, extra_pipe, register, ready_reg, out_stall, codeExitFlag, TotalCycles, pipe2, btbTable1, btbTable2, ExitFlag):
    ...
    if (fetch_ready and end_fetched == 0):
        ...
        print("Final immediate in FETCH is: ",
              immFinal, " in hex: ", hex(immFinal))
        print("rd is = ", rd, "rs1 in FETCH is=",
              rs1, "and rs2 in FETCH is=", rs2)
        # decode_ready = 1
...
def decode(pipe2, out2, register, codeExitFlag):
    ...
    if (decode_ready):
        ...
        operand1, operand2 = op2selectMUX(
            instructionType, rs1, rs2, immFinal, register)
        ...
        printOperationDetails(instructionType, immFinal,
                              operand1, operand2, rd, ALUop)
...
def Write(pipe5, out5, register, pipe1, ready_reg, pipe2, pipe3, pipe4, globalCounter, codeExitFlag,Stats):
    ...
    if (write_ready):
        ...
        if (isBranch == 0):
            ...
            if (pipe1[1] == 0 and end_fetched == 0 and (inst_type == 'J' or inst_type == 'B')):
                # this is to tell that fetch should take pc from write_back
                out5.append(0)
                pipe1[1] = 1
...
def run_riscvsim():
    ...
    with Manager() as manager:
        ...
        mem_ready = 0
        write_ready = 0
        # 1 -> end has been fetched
        end_fetched = 0
        # 1-> read pc from write
        read_pc_from_write = 0
        ...
        '''
         ...
         '''
        ready_reg = mp.Array(
            'i', 32, lock=False)  # Showing if rd Ready to be Read

        for i in range(32):
            ready_reg[i] = 1

        pipe1 = manager.list([pc, fetch_ready, MEM, decode_ready, end_fetched])
        pipe2 = manager.list([pc, opcode, rs1, rs2, rd, func3, func7,
                             immFinal, instructionType, decode_ready, end_fetched])
        pipe3 = manager.list([pc, ALUop, BranchTargetResult, ResultSelect, immFinal, operand1, operand2,
                             rd, MemOp, isBranch, RFWrite, execute_ready, rs2, end_fetched, instructionType, opcode])
        pipe4 = manager.list([pc, MemOp, ALUResult, operand2, RFWrite, ResultSelect, rd, immFinal,
                             isBranch, BranchTargetAddress, mem_ready, rs2, end_fetched, instructionType, opcode])
        pipe5 = manager.list([pc, RFWrite, ResultSelect, rd, immFinal, ReadData, ALUResult,
                             isBranch, BranchTargetAddress, write_ready, end_fetched, instructionType, opcode])
        ...
        for i in range(1500):
            ...
            print("Cycle No.", i+1)
            p1 = mp.Process(target=fetch, args=(pipe1, out1, extra_pipe, register, ready_reg,
                            out_stall, codeExitFlag, TotalCycles, pipe2, btbTable1, btbTable2, ExitFlag))
...
def isBranchInstruction(opcode, inst_type, func3, operand1, operand2):
    ...
    if (opcode == 0b1100111):
        ...
        if (func3 == 0x0 and operand1 == operand2):
            print("operand1==operand2 because operand1=",
...
def printOperationDetails(inst_type, immFinal, operand1, operand2, rd, ALUop):
    ...
    if (inst_type == 'R'):
        ...
        print("immFinal in DECODE=", immFinal, "and operand1 in DECODE=",
              operand1, "and operand2 in DECODE=", operand2)
